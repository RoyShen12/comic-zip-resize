    ...
    seriesDirs.map(async (seriesName) => {
      ...
      await Promise.all(
        chapters.map(async (chapterName) => {
          ...
          const threadId = await zipDirectoryWithThread(
            chapterFullPath,
            chapterZipOutputPath
            // () => console.log(`${chalk.cyanBright(chapterFullName)} ${chalk.greenBright('start zipping')}`),
            // (pointer) =>
            //   console.log(`${chalk.greenBright(chapterFullName)} zip size: ${chalk.blueBright(`${(pointer / 1e6).toFixed(1)} MB`)}`)
          )
